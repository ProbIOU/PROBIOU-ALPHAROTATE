...
def parse_args():
    ...
    parser.add_argument('--checkpoint', dest='checkpoint', help='checkpoint', default='199992', type=str)
    ...
    def worker(self, gpu_id, images, det_net, result_queue):
        ...
        with tf.Session(config=config) as sess:
            ...
            if not restorer is None:
                R = restore_ckpt.split('_')
                restore_ckpt = ""
                for r in R[:-1]:
                    restore_ckpt += (r+'_')
                restore_ckpt += str(self.args.checkpoint)+R[-1][-10:]
