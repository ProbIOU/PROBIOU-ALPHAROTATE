        ...
        if self.is_training:
            with tf.variable_scope('build_refine_loss{}'.format(stage)):
                ...
                if self.cfgs.REG_LOSS_MODE == 2:
                    ...
                    print('\n' + '*'*10 + '\nUsing ProbIoU - L1\n' + '*'*10)
                    mode = 'l1'
                    refine_reg_loss = self.losses.probiou(mode, refine_box_pred, refine_box_states, refine_target_boxes, refine_boxes, is_refine=True)
    ...
    def build_whole_detection_network(self, input_img_batch, gtboxes_batch_h=None, gtboxes_batch_r=None, gpu_id=0):
        ...
        if self.is_training:
            with tf.variable_scope('build_loss'):
                ...
                if self.cfgs.REG_LOSS_MODE == 1:
                    reg_loss = self.losses.wasserstein_distance_loss(rpn_box_pred,
                                                                        anchor_states,
                                                                        target_boxes,
                                                                        anchors, is_refine=False,
                                                                        tau=self.cfgs.GWD_TAU,
                                                                        func=self.cfgs.GWD_FUNC)
                if self.cfgs.REG_LOSS_MODE == 2:
                    reg_loss = self.losses.KL_divergence_loss(rpn_box_pred,
                                                                 anchor_states,
                                                                 target_boxes,
                                                                 anchors, is_refine=False,
                                                                 tau=self.cfgs.KL_TAU,
                    ...
                    print('\n' + '*'*10 + '\nUsing ProbIoU - L1\n' + '*'*10)
                    mode = 'l1'
                    reg_loss = self.losses.probiou(mode, rpn_box_pred, anchor_states, target_boxes, anchors, is_refine=False)
                elif self.cfgs.REG_LOSS_MODE == 4:
                    print('\n' + '*'*10 + '\nUsing ProbIoU - L3\n' + '*'*10)
                    mode = 'l3'
                    reg_loss = self.losses.probiou(mode, rpn_box_pred, anchor_states, target_boxes, anchors, is_refine=False)
